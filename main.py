...
class TranslatorUI(ttk.Window):
    ...
    def _parse_json_with_tolerant_commas(self, text: str) -> tuple[object, bool]:
        normalized, changed = self._strip_trailing_commas_json(text)
        if changed:
            # Treat trailing commas as a tolerated formatting issue.
            # If normalized parsing succeeds, skip any error popup path.
    ...
    def _validate_and_activate_selected_json(self, show_error: bool = False) -> bool:
        ...
        try:
            ...
            data, used_trailing_comma_fallback = self._parse_json_with_tolerant_commas(raw_text)

            self.json_validated = True
            self.cached_json_data = data
            self.cached_json_path = path
            self.status_var.set("JSON validated")
            if used_trailing_comma_fallback:
                self._append_log("JSON validation passed with trailing-comma tolerance.")
    ...
    def _extract_all_keys(self, data) -> set[str]:
        ...
        def has_translatable_string(node) -> bool:
            if isinstance(node, str):
                return bool(node.strip())
            if isinstance(node, list):
                return any(has_translatable_string(item) for item in node)
            if isinstance(node, dict):
                return any(has_translatable_string(v) for v in node.values())
            return False

        def walk(node) -> None:
            if isinstance(node, dict):
                for key, value in node.items():
                    # Include keys that have translatable text directly or inside list/dict values.
                    if has_translatable_string(value):
                        keys.add(str(key))
                    walk(value)
                return
            if isinstance(node, list):
